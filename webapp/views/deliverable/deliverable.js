...
forms.forEach(form => {
    form.addEventListener('submit', e => {
        ...
        formData.append('file', file.files[0] ?? null);
...
.then(documents => {
    ...
    documents.forEach(_document => {
        const id = documents.indexOf(_document);
        createDocumentLine(_document['id'], _document['name'], _document['file_type'], _document['date_deposition'],  `${_document['user']['first_name']} ${_document['user']['last_name']}`, tbody)
        const DOM_download = document.querySelectorAll("a[id*='download']")[id];
        const DOM_delete = document.querySelectorAll("a[id*='delete']")[id];
        download(DOM_download, _document);
        _delete(DOM_delete, _document);
    });
})

function createDocumentLine(id, filename, type, date, user, parent) {
